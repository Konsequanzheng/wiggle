 ...
 * 实现有机的弹性运动效果
 */
function SkeletonClothPhysics({ url }: { url: string }) {
  const group = useRef<THREE.Group>(null);
  const { scene } = useGLTF(url);
  const threeState = useThree();
  const { camera, raycaster } = threeState || {};
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      if (child instanceof THREE.Mesh) {
        ...
        console.log(
          `Virtual skeleton initialized with ${virtualBones.current.length} bones`
        );
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    const targetTilt =
      Math.sign(deltaX) * Math.min(Math.abs(deltaX) * 0.01, maxTilt);
  ...
  useFrame((state, delta) => {
    if (
      ...
      !vertexVelocities.current ||
      !vertexAccelerations.current
    )
      return;
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      // 风力 (使用噪声模拟自然风)
      const windX =
        Math.sin(time * physics.windFrequency + i * 0.1) * physics.windStrength;
      const windZ =
        Math.cos(time * physics.windFrequency * 0.7 + i * 0.15) *
      ...
      const wrinkleX =
        Math.sin(time * 1.5 + i * wrinkleFreq + positionAttr.array[idx] * 5) *
        wrinkleAmp;
      const wrinkleY =
        Math.cos(
          time * 1.2 + i * wrinkleFreq * 0.8 + positionAttr.array[idx + 1] * 5
        ) * wrinkleAmp;
      const wrinkleZ =
        Math.sin(
          time * 1.3 + i * wrinkleFreq * 1.2 + positionAttr.array[idx + 2] * 5
        ) * wrinkleAmp;
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      for (let j = i + 1; j < Math.min(i + neighborRange, vertexCount); j++) {
        ...
        const origDist = Math.sqrt(
          origDx * origDx + origDy * origDy + origDz * origDz
        );
      ...
      // 添加剪切约束 (对角顶点)
      for (
        let j = i + neighborRange;
        j < Math.min(i + neighborRange * 2, vertexCount);
        j += 3
      ) {
        const jdx = j * 3;
        const jx = positionAttr.array[jdx];
    ...
    if (isDragging && dragPoint.current) {
      ...
      for (let i = 0; i < vertexCount; i++) {
        ...
        if (dist < dragRadius) {
          ...
          vertexAccelerations.current[idx] +=
            ((x - dragPoint.current.x) / dist) *
            radialForce *
            Math.sin(time * 3.0);
          vertexAccelerations.current[idx + 2] +=
            ((z - dragPoint.current.z) / dist) *
            radialForce *
            Math.cos(time * 3.0);
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      vertexVelocities.current[idx + 1] +=
        vertexAccelerations.current[idx + 1] * dt;
      vertexVelocities.current[idx + 2] +=
        vertexAccelerations.current[idx + 2] * dt;
...
export default function ModelViewer() {
  return (
    ...
    >
      ...
      <SkeletonClothPhysics url="/model.glb" />
